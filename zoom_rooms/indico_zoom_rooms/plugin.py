...
def only_zoom_rooms(f):
    @wraps(f)
    def _wrapper(
        obj: Event | Contribution | SessionBlock | VCRoomEventAssociation, *args: tuple, **kwargs: dict
    ):
...
@only_zoom_rooms
def _delete_zoom_association(
    old_link: SessionBlock | Contribution | Event, vc_room: VCRoom
):
...
def _handle_contribution_move(
    obj: Contribution, original_block: SessionBlock, new_block: SessionBlock
):
    """Contribution moved between blocks"""
...
def _signal_link_object_updated(
    obj: Session | SessionBlock | Contribution | Event, changes: dict | None = None
):
    _check_link_object_for_updates(obj, changes)


def _signal_tt_entry_updated(
    obj_type, entry: TimetableEntry, obj: Contribution | SessionBlock, changes: dict
):
    _check_link_object_for_updates(obj, changes)


def _signal_zoom_meeting_created(
    vc_room: VCRoom, assoc: VCRoomEventAssociation, event: Event
):
...
def _signal_zoom_meeting_cloned(
    ...
    # Zoom Rooms: on clone, clone also the zoom room calendar entry
        ...
        for assoc in vc_room.events:
            if zr_id := get_zoom_room_id(assoc.link_object):
                ZoomRoomsQueueEntry.record(
                    ZoomRoomsAction.update, zr_id, assoc=assoc, args=OperationArgs(title=name)
                )
...
class ZoomRoomsPlugin(IndicoPlugin):
    ...
    default_settings = {'debug': False, 'service_url': None, 'token': None, 'timeout': 3}
    ...
    def init(self):
        ...
        self.connect(signals.event.session_updated, _signal_link_object_updated)
